...
<details>
  <summary>Stateful Set: AWS EFS CSI Driver </summary>
...
aws efs describe-file-systems --file-system-id $FILE_SYSTEM_ID  # LifeCycleState: creating -> availabe
```	
#### Create mount on in each subnet
```
...
```
```	
aws efs describe-mount-targets --file-system-id $FILE_SYSTEM_ID | jq --raw-output '.MountTargets[].LifeCycleState'  # Wait: LifeCyclceState -> availabe 	
```
...
kubectl get pvc -n storage    # should see: Bound
kubectl get pv	
```	
#### Deploy app
```
...
```	
```
### note: there might be more than one FS in place. FileSystems[] -> FileSystems[0] or [1]
FILE_SYSTEM_ID=$(aws efs describe-file-systems | jq --raw-output '.FileSystems[].FileSystemId')
echo $FILE_SYSTEM_ID  # make sure it returns only 1 entry that you want to delete (otherwise, might delete the wrong one)	
```
```	
...
aws efs describe-file-systems --file-system-id $FILE_SYSTEM_ID  # check to see if mount deleted. NumberOfMountTargets field in the JSON output reads 0
```	
```
...
```
eksctl version  #v >0.58.0  	
```
#### VPC resource controller and VPC admission controller webhook
```
eksctl utils \
    ...
    --approve
```
#### Create Self-Managed Windows Nodes	
```
...
```	
```
kubectl get nodes -l kubernetes.io/os=windows -L kubernetes.io/os   # check EC2 instance to see windows-ng-Node
```	
#### NodeSelector
```
# Linux
...
echo http://${WINDOWS_IIS_SVC}   # check in the browser
```	
#### Deploy Calico 
```
kubectl apply -f https://raw.githubusercontent.com/aws/amazon-vpc-cni-k8s/release-1.6/config/v1.6/calico.yaml
```
```
kubectl get daemonset calico-node --namespace=kube-system   # install the calico-node on Linux Only
```
#### for Windows we need to add a rolebinding to allow the windows nodes to install Calico.	
```
...
```	
#### Install Calico on Windows Node
```
kubectl get nodes -l kubernetes.io/os=windows -L kubernetes.io/os
```
#### Install kubectl on Windows Node	
```	
# find the Windows instance on EC2 console: connect SSM
mkdir c:\k
...
kubectl -n windows exec -it <nginx-pod-name> -- /bin/bash
  ping <windows-pod-ip>  # should ok
```	
```
kubectl -n windows exec -it <windows-pod-name> -- powershell
  ping <nginx-pod-ip>   # shold ok
```	
